...
/*
      ...
      */
      ...
      List<PeerConnection.IceServer> iceServers = getTurnServers();
      ...
      //  SignalingParameters params = new SignalingParameters(iceServers, true, "86673702", "wss://apprtc-ws.webrtc.org:443/ws", "https://apprtc-ws.webrtc.org:443", offerSdp, iceCandidates);
  ...
  private List<PeerConnection.IceServer> requestTurnServers(String url) throws IOException, JSONException {
    ...
    //  connection.setRequestProperty("REFERER", "https://i-webscout-dev.ityx.de/chat-test/video_m/video.html");
  ...
  private List<PeerConnection.IceServer> iceServersFromPCConfigJSON(String pcConfig) throws JSONException {
    ...
    for (int i = 0; i < servers.length(); ++i) {
      ...
      String credential = server.has("credential") ? server.getString("credential") : "";
      PeerConnection.IceServer turnServer = PeerConnection.IceServer.builder(url).setPassword(credential).createIceServer();
      ret.add(turnServer);
    }
    return ret;
  }
  ...
  // Requests & returns a TURN ICE Server based on a request URL.  Must be run
  // off the main thread!
  private List<PeerConnection.IceServer> getTurnServers() throws JSONException {
